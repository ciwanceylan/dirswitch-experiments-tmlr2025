...
class Generator:
    def __init__(self, n_node, node_emd_init, config):
        ...
        self.gen_w_1 = tf.get_variable(
            name="gen_w",
            shape=[2, self.emd_dim, self.emd_dim],
            initializer=tf.contrib.layers.xavier_initializer(uniform=False),
            trainable=True,
        )
        self.gen_b_1 = tf.get_variable(
            name="gen_b",
            shape=[2, self.emd_dim],
            initializer=tf.contrib.layers.xavier_initializer(uniform=False),
            trainable=True,
        )
        self.gen_w_2 = tf.get_variable(
            name="gen_w_2",
            shape=[2, self.emd_dim, self.emd_dim],
            initializer=tf.contrib.layers.xavier_initializer(uniform=False),
            trainable=True,
        )
        self.gen_b_2 = tf.get_variable(
            name="gen_b_2",
            shape=[2, self.emd_dim],
            initializer=tf.contrib.layers.xavier_initializer(uniform=False),
            trainable=True,
        ...
        self.dis_node_embedding = tf.placeholder(
            tf.float32, shape=[2, None, self.emd_dim]
        )

        self.node_embedding = tf.nn.embedding_lookup(
            self.node_embedding_matrix, self.node_ids
        )

        _noise_embedding = []
        for i in range(2):
            _noise_embedding.append(
                tf.reshape(
                    tf.nn.embedding_lookup(self.noise_embedding, tf.constant([i])),
                    [-1, self.emd_dim],
                )
            )
        _dis_node_embedding = []
        for i in range(2):
            _dis_node_embedding.append(
                tf.reshape(
                    tf.nn.embedding_lookup(self.dis_node_embedding, tf.constant([i])),
                    [-1, self.emd_dim],
                )
            )
        ...
        for i in range(2):
            _fake_node_embedding = self.generate_node(
                self.node_embedding, _noise_embedding[i], i
            )
            _fake_node_embedding_list.append(_fake_node_embedding)

            _score[i] = tf.reduce_sum(
                tf.multiply(_dis_node_embedding[i], _fake_node_embedding), axis=1
            )

            _neg_loss[i] = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(
                    labels=tf.ones_like(_score[i]) * (1.0 - config.label_smooth),
                    logits=_score[i],
                )
            ) + config.lambda_gen * (
                tf.nn.l2_loss(self.node_embedding) + tf.nn.l2_loss(self.gen_w_1[i])
            )
    ...
    def generate_node(self, node_embedding, noise_embedding, direction):
        ...
        output = tf.nn.leaky_relu(
            tf.matmul(input, self.gen_w_1[direction]) + self.gen_b_1[direction]
        )
