...
from .gat import GAT
from .dirgat import RossiDirGAT
from .graphsage import GraphSAGE
from .dirgraphsage import RossiDirGraphSAGE
from .switch_model_graphsage import SAGEMultiSwitchModel
from .switch_model_gat import GATMultiSwitchModel
...
def setup_module(
    ...
    **kwargs,
    ...
    if not directed:
        if m_type.startswith("graphsage"):
            mod = GraphSAGE(
                in_dim=in_dim,
        ...
        match m_type:
            case "graphsage_switch":
                mod = SAGEMultiSwitchModel(
                    dir_seqs=dir_seqs if is_encoding else ("U",),
                    in_dim=in_dim,
                    num_hidden=num_hidden,
                    out_dim=out_dim,
                    num_layers=num_layers,
...
class PreModel(nn.Module):
    def __init__(
        ...
        if encoder_type.startswith("gat"):
            enc_num_hidden = num_hidden // nhead

        if decoder_type.startswith("gat"):
